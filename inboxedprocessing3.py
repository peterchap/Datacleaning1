...
file1 = 'LM ESB UK 25.09.2019 .csv'
file2 = 'LM ESB UK 25.09.2019_all_data_flag.csv'
...
to_dropremove = ['first_name', 'last_name', 'date_of_birth', 'gender', 'email',\
'ipaddress', 'timestamp', 'source', 'is_duplicate', 'is_ok', 'domain', 'user_status', 'last_open',\
'last_click', 'system_created',  'email_id',\
'primary_membership_id', 'primary_membership', 'name', 'rtotal']
...
for i, x in df.groupby(['gender']):
        for j, m in x.groupby('data flag'):
            ...
            output = pd.DataFrame(columns=['EMAIL', 'TITLE', 'FIRSTNAME', 'LASTNAME', 'ADDRESS1',\
            'ADDRESS2', 'ADDRESS3', 'CITY', 'COUNTY', 'POSTCODE', 'PHONE', 'MOBILE', 'GENDER', 'DOB', 'URL', 'IP', 'JOINDATE'])
            output['EMAIL'] = m['email']
            output['FIRSTNAME'] = m['first_name']
            output['LASTNAME'] = m['last_name']
            output['DOB'] = m['date_of_birth']
            output['GENDER'] = m['gender']
            output['IP'] = m['ipaddress']
            output['URL'] = m['source']
            output['JOINDATE'] = m['timestamp']
            output.to_csv(directory +  i + "_"+ j + month + ".csv", index=False)
